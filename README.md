...
一屏搞定：贴后台链接 + 多语言 JSON → 勾选要填的项 → 开始填充 → 看实时日志。界面中英双语随手切。
...
- **注意**：AMO 的描述编辑不走草稿，保存即生效（不触发重新审核）；Chrome / Edge 仍只写草稿。
...
3. 在控制台里：
   - 贴后台链接（Chrome 的 `…/edit`、Edge 的 `…/listings`、Firefox 的 `…/developers/addon/<名>/edit`，至少一个）+ 描述 JSON（可选再填搜索词 JSON），点**保存**。
   - 点**登录** → 在弹出的浏览器登录 Google / Microsoft / Mozilla（只需一次，之后免登录）。
   - **勾选要填的项**（Chrome 描述 / Edge 描述 / Edge 搜索词 / Firefox 描述，可多选，缺链接或内容的项自动置灰）→ 点**开始填充** → 看实时日志 → 去浏览器人工检查 → 自己提交。

> 右上角 **中 / EN** 切界面语言（会记住）。运行日志由服务端产出，目前仅中文。

## 多项目

维护多个扩展？控制台顶部可以新建 / 切换 / 重命名 / 删除项目，每个项目独立保存三个后台链接与两份文案 JSON（存放在 `projects/<项目名>/`，已 git-ignore）。老用户首次升级启动时，原有的链接与文案会自动迁移到 `projects/default/`。

命令行同样支持：`node playwright/run.mjs --store all --project 项目名`（缺省用控制台当前选中的项目）。
...
- 不确定怎么写？控制台里点 **「填入样例」** 载入可编辑模板，或复制仓库里的 `descriptions.example.json` 改。也可以把 [`store-copy-prompt.md`](store-copy-prompt.md) 里的提示词交给 AI 助手，在你的扩展项目里一次生成合规的描述与搜索词两份文件。
- 描述里要换行就写成 `\n`（JSON 字符串里不能直接回车换行）。
...
> 链接与文案都在 `projects/`（连同根上的 `config.json`）里，均已 git-ignore，不会被提交。
...
```bash
npm run login    # 首次登录（打开各后台，登录完 Ctrl+C）
npm run chrome   # 只填 Chrome
npm run edge     # 只填 Edge
npm run firefox  # 只填 Firefox (AMO)
npm run all      # 全部
# 指定项目：node playwright/run.mjs --store all --project 项目名
```

## 结构与扩展

- `playwright/` — 填充逻辑（`fill-chrome.mjs` / `fill-edge.mjs` / `fill-firefox.mjs` 描述、`fill-edge-terms.mjs` 搜索词）
- `gui/` — 本地控制台（`gui/server.mjs` 启动；前端 React + antd，源码 `gui/web/src`）
- `src/` — 共享纯逻辑：`core.mjs`（解析 / 队列 / URL 语言 / 搜索词清洗，含单测）、`selectors.mjs`（选择器与中英名表）、`projects.mjs`（多项目存储，含单测）
